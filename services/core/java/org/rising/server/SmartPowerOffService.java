    ...
    private void updateShutDownTime() {
        ...
        if (timeValue != null) {
            ...
            if (parts.length == 2) {
                ...
                long powerOffTimeInMillis = calendar.getTimeInMillis();
                // If the user accidentally sets the power-off time to now or a past time, schedule it for the next day
                if (powerOffTimeInMillis <= currentTimeMillis) {
                    calendar.add(Calendar.DAY_OF_YEAR, 1);
                    powerOffTimeInMillis = calendar.getTimeInMillis();
                    Log.w(TAG, "Power-off time set to now or past; rescheduled for tomorrow.");
                }
                mPowerOffTimeMillis = powerOffTimeInMillis;
                mDelayEndTimeMillis = mPowerOffTimeMillis;
                mAlarm.set(powerOffTimeInMillis);
            }
        }
    }
